...
impl Display for MeasureAnalysis {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    let tangles = self
      .entangled_with
      .iter()
      .map(|val| format!("Q{}~{}", val.qubit, val.ratio))
      .collect::<Vec<_>>();
...
impl AnalysisQubit {
  ...
  /// Retrieve the measurement information about this qubit but don't snap the tangles.
  ...
  /// Measures the qubit, snapping all tangles to their appropriate poles.
  pub fn measure(&self) -> MeasureAnalysis {
    let results = self.analyze_measure();
    if !self.tangles.is_empty() {
      for (key, tangle) in self.tangles.iter() {
  ...
  /// Sympathetically snaps resultant values to 1/0 due to entangled qubit being measured.
  pub fn snap(&self) {
...
impl EntanglementCluster {
  ...
  pub fn remove(&self, index: &i64) {
    ...
    if let Some(qubit) = self.qubits.get(&index) {
      if self.is_tracing() {
        log!(Level::Info, "Removing {} from cluster: {}.", index, self)
      }
  ...
  pub fn measure(&self, index: &i64) -> MeasureAnalysis {
    let qubit = with_mutable_self!(self.qubits.remove(&index).expect(&format!(
...
pub struct SolverResultsSynthesis {
...
impl SolverResultsSynthesis {
  pub fn new(probability_range: f64, max_entangles_per_add: usize) -> SolverResultsSynthesis {
    SolverResultsSynthesis {
      probability_range,
  ...
  pub fn add(&mut self, measure: &MeasureAnalysis) {
    ...
    let highest_probability = measure.probability;
    ...
    for ent_meta in entanglements.iter() {
      ...
      for fragment in results.iter() {
        ...
        new_fragment.add(ent_meta.qubit, qubit_result, fragment.rolling_probability);
  ...
  pub fn combine(&self) -> Vec<SolverResult> {
    let mut results = Vec::new();
    for fragment in self.fragments.iter() {
      let mut composite = fragment.clone();
      for overlay_fragment in self.fragments.iter() {
...
impl Display for SolverResultsSynthesis {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    f.write_str(&format!(
      "\n{}",
      &self
...
impl QuantumSolver {
  ...
  pub fn solve(&self) -> Vec<SolverResult> {
    ...
    let mut synth = SolverResultsSynthesis::new(self.probability_range, self.max_entanglements);
    for (key, value) in self.measures.iter() {
      synth.add(value);
    }

    log!(Level::Info, "Synthesis pre-combine: {}", synth);
    let results = synth.combine();
    log!(Level::Info, "Synthesis post-combine: {}", synth);
...
mod tests {
  ...
  #[test]
  fn bell_test() {
    let solver = QuantumSolver::with_trace(Ptr::from(TracingModule::with(ActiveTracers::Solver)));
