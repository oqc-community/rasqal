...
fn scope_variables_rec(graph: &Ptr<AnalysisGraph>, context: &Ptr<RuntimeContext>, guard: &mut HashSet<String>) {
    ...
    with_mutable!(context.scopes.insert(graph.identity.clone(), Ptr::from(HashMap::new())));
    ...
    for node in walk_logical_paths(graph) {
        ...
        let backward_jumps = inc_nodes.iter().filter(|val|
          val.1.order.expect("Should be ordered") > node_order).map(|val| &val.1).collect::<Vec<_>>();
        ...
        if !active_scopes.is_empty() {
            match node.instruction.deref() {
                ...
                Instruction::Expression(_, var_opt) |
    ...
    match next_target.0.assignments.as_ref() {
        Some(assignments) => {
            for (assign, value) in assignments {
                ...
                //  jumps.
                with_mutable!(context.add(assign, &follow_reference(&Ptr::from(value.clone()), context)));
            }
        }, _ => {}
    }
...
fn follow_reference(qx: &Ptr<Value>, context: &Ptr<RuntimeContext>) -> Ptr<Value> {
    match qx.deref() {
        Value::Ref(ref_, additional) => {
            let fetched_ref = context.get(ref_).expect(format!("Dangling variable: {}.", ref_.clone()).as_str());
            ...
            additional.as_ref().map_or(value.clone(), |indexer | {
                match value.deref_mut() {
                    ...
                    _ => panic!("Tried indexer on value that wasn't an array: {}.", value.to_string())
                }
            })
        },
        _ => qx.clone()
    ...
    pub fn execute(&self, context: &Ptr<RuntimeContext>) -> Ptr<Value> {
        match self {
            Expression::Clone(value) => {
                follow_reference(&Ptr::from(value), context).clone()
            }
            ...
            Expression::ArgInjection(target, args) => {
                ...
                if let Some(args) = args {
                    follow.argument_mappings.insert("%arg-tuple".to_string(), Ptr::from(args.clone()));
                }
            ...
            Expression::MakeCtrlAdj(val, modifier) => {
                ...
                let slimmed_id = id.trim_end_matches("__ctrl__wrapper").trim_end_matches("__adj__wrapper");

                let suffix = match modifier {
                    LambdaModifier::Ctl => if is_adj { "__ctladj__wrapper" } else { "__ctl__wrapper" },
                    LambdaModifier::Adj => if is_controlled { "__ctladj__wrapper" } else { "__adj__wrapper" }
                };
                ...
                if let Some(new_graph) = context.method_graphs.get(suffix) {
                    ...
                    panic!("Attempted swapping graph out for adjoint or controlled version, can't find wrapper.");
                }
...
impl TracingModule {
    pub fn new() -> TracingModule {
        TracingModule { tracers: ActiveTracers::empty() }
    }

    pub fn with(tracers: ActiveTracers) -> TracingModule {
        TracingModule { tracers: tracers.clone() }
    }
...
impl QuantumRuntime {
    ...
    pub fn execute(&mut self, exe_graph: &Ptr<ExecutableAnalysisGraph>, arguments: &Vec<Value>) -> Result<Option<Ptr<Value>>, String> {
        let mut context = exe_graph.context.attach_runtime(&Ptr::from(self.borrow_mut()));
        ...
        if exe_graph.callable_graph.argument_mappings.len() != arguments.len() {
            let mut required_arguments = exe_graph.callable_graph.argument_mappings.keys()
              .map(|val| val.clone()).collect::<Vec<_>>().join(", ");

            if required_arguments.is_empty() {
                required_arguments = String::from("no");
            }

            let mut supplied_arguments = arguments.iter().map(|val| val.to_string()).collect::<Vec<_>>().join(", ");
        ...
        self._execute(exe_graph.callable_graph.analysis_graph.borrow(), &mut context).map(|val| {
            if val.is_none() {
                return None;
            }
            ...
            Some(match val.deref() {
                Value::QuantumPromise(qbs, proj) => {
                    Ptr::from(Value::AnalysisResult(Ptr::from(with_mutable!(proj.results_for(qbs)))))
                }
                Value::Array(arr) => Ptr::from(Value::Array(arr.iter()
    ...
    fn _execute(&mut self, graph: &Ptr<AnalysisGraph>, context: &mut Ptr<RuntimeContext>) -> Result<Option<Ptr<Value>>, String> {
        ...
        loop {
            if self.is_tracing() {
                ...
                for (key, value) in context.variables.iter() {
                    ...
                    if changed {
                        changed_variables.push(format!("({} = {})", key.clone(), followed_value.to_string()));
                    }
                    ...
                    let copied_value = match followed_value.deref() {
                        Value::Array(arr) => {
                            Ptr::from(Value::Array(arr.iter().map(|val| val.clone_inner()).collect::<Vec<_>>()))
                        },
                        _ => value.clone_inner()
                    };
                ...
                log!(Level::Info, "{} :: {}", current_node.to_string().as_str(), changed_variables.join(", "));
            }
            ...
            if let Some(scopes) = available_scopes.as_mut() {
                if let Some(scope) = scopes.get_mut(&current_node.order.expect("Node ordering required.")) {
                    ...
                    // Else we've looped, so remove the scoped variables we already know about.
                        ...
                        if self.is_tracing() {
                            log!(Level::Info, "Looped, resetting [{}]", scope.captured_variables.iter().map(|val| val.as_str()).collect::<Vec<_>>().join(", "))
                        }
            ...
            match instruction.deref() {
                ...
                Instruction::Throw(message) => {
                    return Err(message.as_ref().map_or("Unknown exception.".to_string(), |val|
                ...
                Instruction::Subgraph(subgraph, var) => {
                    ...
                    for (arg, value) in subgraph.argument_mappings.iter() {
                        ...
                        subcontext.variables.insert(
                            arg.clone(), follow_reference(&value, context).clone());
                    }
                    ...
                    if let Some(target) = var {
                        let results = results.map_or(
                            Ptr::from(Value::Empty),
                            |val| val.clone()
                        );
                ...
                Instruction::Assign(variable, val) => {
                    ...
                    if let Value::Callable(callable) = cloned_value.deref() {
                        ...
                        for key in callable.argument_mappings.keys().clone() {
                            with_mutable!(callable.argument_mappings.insert(key.clone(), follow_reference(callable.argument_mappings.get(key).unwrap(), context)));
                        }
                    }
                    ...
                    match gate.deref() {
                        ...
                        Gate::CR(pauli, controls, target, rotation) => {
                            ...
                            let controls = match follow_reference(controls, context).deref() {
                                ...
                                Value::Array(arr) => arr.iter().map(|val| follow_qubit(val, context).clone()).collect(),
                                _ => Vec::new()
                            };
                        ...
                        Gate::Measure(pauli, qbs, var) => {
                            let qubits = match follow_reference(qbs, context).deref() {
                                Value::Qubit(qb) => {vec![qb.clone()]}
                                Value::Array(array) => {
                                    array.iter().map(|val| follow_reference(val, context).as_qubit().clone()).collect::<Vec<_>>()
                                }
                                _ => panic!("Invalid qubit.")
                            };

                            let mut projection = context.activate_projection(
                                &qubits.iter().next().expect("Should have at least one qubit to measure."));
                            projection.add(&Ptr::from(QuantumOperations::Measure(qubits.clone())));

                            let paulis = match follow_reference(pauli, context).deref() {
                                Value::Array(array) => {
                                    array.iter().map(|val| follow_reference(val, context).as_pauli()).collect::<Vec<_>>()
                                },
                                val => vec![val.as_pauli()]
                            };

                            let promise = Ptr::from(
                                Value::QuantumPromise(qubits, projection.clone()));
        ...
        if context.is_base_profile {
            if let Some(projection) = context.projections.values_mut().next() {
                return Ok(Some(Ptr::from(Value::AnalysisResult(Ptr::from(projection.results())))));
            }
        }
...
impl Display for VariableScopes {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(format!("at {} with {}", self.start, self.captured_variables.iter().map(|val| val.as_str()).collect::<Vec<_>>().join(", ")).as_str())
    }
}
