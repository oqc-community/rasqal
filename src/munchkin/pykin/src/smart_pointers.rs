...
impl<T: ?Sized> FlexiPtr<T> {
    ...
    pub fn is_not_null(self_: &Self) -> bool {
...
impl<T: ?Sized + Clone> FlexiPtr<T> {
    ...
    pub fn clone_inner(&self) -> FlexiPtr<T> {
        Ptr::from(self.deref().clone())
    }
}
...
impl<T: ?Sized> From<&T> for FlexiPtr<T> {
    fn from(value: &T) -> Self {
        FlexiPtr::Borrow(value as *const T as *mut T)
    }
}
...
mod tests {
        ...
        unsafe {
            ...
            unsafe fn nested_calls(arg: &FlexiPtr<Recursive>) {
                ...
                unsafe fn further_nested_calls(arg: &FlexiPtr<Recursive>) {
                    ...
                    assert_eq!(FlexiPtr::as_address(arg), FlexiPtr::as_address(&another_pointer));
    ...
    fn recursive_replace() {
        let mut starter = FlexiPtr::from(Recursive { nested_flexi: Default::default(), value: "Dave".to_string() });
        let second = FlexiPtr::from(Recursive { nested_flexi: Default::default(), value: "Dave".to_string() });
    ...
    fn complicated_expansion() {
        let starter = FlexiPtr::from(Recursive { nested_flexi: Default::default(), value: "Dave".to_string() });
        ...
        let second = FlexiPtr::from(Recursive { nested_flexi: Default::default(), value: "Dave the second".to_string() });
        ...
        for i in 0..20 {
            slist.get_mut(i).unwrap().expand_into(&dlist.get_mut(i).unwrap());
        }
        ...
        for val in slist.iter() {
            ...
            assert_eq!(val.ref_count().expect("Should be a reference"), second.ref_count().expect("Should be a reference"));
        }
