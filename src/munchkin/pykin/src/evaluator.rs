...
use llvm_sys::core::{LLVMConstIntGetSExtValue, LLVMGetElementType, LLVMGetNumOperands, LLVMGetOperand, LLVMGetTypeKind, LLVMPrintTypeToString, LLVMPrintValueToString, LLVMTypeOf};
use llvm_sys::LLVMTypeKind;
use crate::with_mutable;
use crate::graphs::{Node, AnalysisGraph, AnalysisGraphBuilder, ExecutableAnalysisGraph, CallableAnalysisGraph};
use crate::hardware::Qubit;
use crate::instructions::{Condition, Equalities, Expression, Instruction, LambdaModifier, Operator, Pauli, Value};
use crate::runtime::RuntimeContext;
use crate::smart_pointers::{Ptr};

macro_rules! operand_to_value {
    ($target:ident, $index:expr) => ($target.get_operand($index).expect("Can't resolve operand.").left().expect("Operand isn't a value.").as_any_value_enum().borrow_mut());
}

macro_rules! operand_to_instruction {
    ($target:ident, $index:expr) => ($target.get_operand($index).expect("Can't resolve operand.").left().expect("Operand isn't a value."));
}

macro_rules! operand_to_bb {
    ($target:ident, $index:expr) => ($target.get_operand($index).expect("Can't resolve operand.").right().expect("Operand isn't a basic block."));
}
...
pub fn parse_ref_id_from_instruction_str(inst_str: &String) -> Option<String> {
    ...
    llvm_var_finder.captures(inst_str.as_str())
      .map_or_else(
...
pub fn parse_ref_id_from_value(ptr_string: String) -> Option<String> {
    ...
    let mut ref_id = capture_groups.map_or(None, |val| {
        Some(val.get(1).unwrap().as_str().to_string())
    });
...
impl<'a> EvaluationContext<'a> {
    ...
    pub fn next_throwaway(&self) -> String {
        ...
        unsafe {
            self.throwaway_variables.as_ptr().replace(*self.throwaway_variables + 1);
        }
        var
    }
}
...
impl QIREvaluator {
    pub fn new() -> QIREvaluator {
        QIREvaluator {}
    }

    /// Evaluates a module and entry-point for execution. Returns the execution graph to then be
    /// run in an interpreter.
    pub fn evaluate(&self, entry_point: &FunctionValue, module: &Ptr<Module>) -> Result<Ptr<ExecutableAnalysisGraph>, String> {
        ...
        while target_global.is_some() {
            ...
            if let Some(init) = maybe_initializer {
                ...
                if let Some(value) = self.as_value(&init.as_any_value_enum(), &Ptr::default(), context.borrow()) {
        ...
        for param in entry_point.get_params().iter() {
            ...
            callable.argument_mappings.insert(param_ref_id, Ptr::from(Value::Empty));
        }

        let exe_graph = ExecutableAnalysisGraph::with_context(
            &callable, &Ptr::from(RuntimeContext::from_evaluation(&context)));
    ...
    fn walk_basic_block(&self, bb: &BasicBlock, graph: &Ptr<AnalysisGraphBuilder>,
        ...
        let starting_point = with_mutable!(context.anchors.get_mut(bb_name).expect("Anchor needs to exist."));
    ...
    fn walk_function(&self, func: &FunctionValue,
                     context: &Ptr<EvaluationContext>) -> Ptr<AnalysisGraphBuilder> {
    ...
    fn extract_gep(&self, any_val: &AnyValueEnum, graph: &Ptr<AnalysisGraphBuilder>,
        ...
        unsafe {
            ...
            let type_string = CStr::from_ptr(LLVMPrintTypeToString(type_ref)).to_str().unwrap().to_string();
            ...
            let ref_id = parse_ref_id_from_instruction_str(&llvm_string.to_string_lossy().to_string()).expect("Need ref-id from instruction");
            ...
            while starting_operand < op_num {
                ...
                graph.Assign(next_throwaway.clone(), Value::Ref(prev_throwaway.clone(), Some(Ptr::from(Value::Int(actual_index)))));
        ...
        match type_enum {
            AnyTypeEnum::ArrayType(t) => {
                ...
                if vec.is_const_string() {
                    Some(Value::String(vec.get_string_constant().unwrap().to_str().unwrap().to_string()))
                } else if vec.is_const() {
                    let mut result = Vec::new();
                    for int in 0..(t.len()) {
                        result.push(
                            self.as_value_ptr(
                                &vec.const_extract_value(&mut [int]).as_any_value_enum(), graph, context).expect("Can't resolve array element."));
                    }
            ...
            AnyTypeEnum::FloatType(t) => {
                ...
                let numeric = val_enum.into_float_value().get_constant().expect("Float parsing has failed.").0;
            ...
            AnyTypeEnum::IntType(t) => {
                let llvm_context = t.get_context();
                let numeric = val_enum.into_int_value().get_sign_extended_constant().expect("Int parsing has failed.");
            ...
            AnyTypeEnum::StructType(t) => {
                ...
                if t.is_opaque() {
                    let struct_name = t.get_name().unwrap_or_default().to_str().unwrap_or_default();
                    let ptr_val = val_enum.into_pointer_value();
                    let index = if ptr_val.is_null() || ptr_val.is_undef() {
                        0
                    } else {
                        ptr_val.const_to_int(context.module.get_context().i64_type())
                          .get_sign_extended_constant().unwrap_or_default() as i64
                    };
                ...
                // TODO: Decide whether arrays/composites need to be distinguished.
                    ...
                    for int in 0..(t.count_fields()) {
                        result.push(
                            self.as_value_ptr(
                                &struct_val.const_extract_value(&mut [int]).as_any_value_enum(), graph, context).expect("Can't resolve struct element."));
                    }

                    Some(Value::Array(result))
                }
            },
            AnyTypeEnum::VectorType(_) => { unimplemented!() },
            AnyTypeEnum::VoidType(_) => { unimplemented!() },
            AnyTypeEnum::FunctionType(_) => { unimplemented!() },
        }
    }

    /// See [as_value] but returns value wrapped in a flexi-pointer.
    fn as_value_ptr(&self, any_val: &AnyValueEnum,
                    graph: &Ptr<AnalysisGraphBuilder>,
                    context: &Ptr<EvaluationContext>) -> Option<Ptr<Value>> {
    ...
    fn as_value(&self, any_val: &AnyValueEnum,
                graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) -> Option<Value> {
        let function_val = match any_val {
            AnyValueEnum::FunctionValue(fv) => Some(Value::Ref(fv.get_name().to_str().unwrap().to_string(), None)),
            _ => None
        };
    ...
    fn walk_instruction(&self, inst: &Ptr<InstructionValue>,
        ...
        match op_code {
            ...
            _ => panic!("Unknown instruction type: {}! Can't verify program will execute correctly.", inst.print_to_string().to_string())
        }
    }
    ...
    fn eval_intrinsic(&self, name: String, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) -> Option<Value> {
        ...
        let parse_default_callable = |global_name: &String| -> Option<Ptr<AnalysisGraph>> {
            context.global_variables.get(global_name).map_or(None, |callable_array| {
                let mut first = None;
                        ...
                        if let Some(llvm_method) = context.module.get_function(method_name.as_str()) {
                            ...
                            if first_eval {
                                for exit in builder.exit_points() {
                                    let ret_node = Ptr::from(Instruction::Return(Ptr::from(Value::Ref("%result-tuple".to_string(), None))));
        ...
        let expand_arg_tuple = |tuple_index: u32|-> (String, String, String) {
            ...
            graph.Assign(pauli.clone(), Value::Ref(tuple_var.clone(), Some(Ptr::from(Value::Int(0)))));
            let rotation = context.next_throwaway();
            graph.Assign(rotation.clone(), Value::Ref(tuple_var.clone(), Some(Ptr::from(Value::Int(1)))));
             let target = context.next_throwaway();
            graph.Assign(target.clone(), Value::Ref(tuple_var, Some(Ptr::from(Value::Int(2)))));
        ...
        match name.as_str() {
            ...
            "__quantum__qis__r__ctladj" => {
                ...
                graph.Expression(Expression::NegateSign(Value::Ref(rotation, None)), Some(throwaway.clone()));
                graph.CR(
                    Value::Pauli(Pauli::Z),
                    controls,
                    Value::Ref(target, None),
            ...
            "__quantum__rt__array_get_element_ptr_1d" => {
                ...
                let target = parse_as_value(inst, 0)
                  .expect("Target of array access unresolvable.").as_reference();
            ...
            "__quantum__rt__callable_copy" => {
                let ref_id = get_ref_id_from_instruction(inst.borrow());
                 let val = parse_as_value(inst, 0)
                  .expect("Can't resolve value.");
            ...
            "__quantum__rt__callable_create" => {
                ...
                if let Some(lambda) = callable_lambda {
                    ...
                    subgraph.argument_mappings.insert("%capture-tuple".to_string(), Ptr::from(stored));
                    subgraph.argument_mappings.insert("%result-tuple".to_string(), Ptr::from(Value::Array(Vec::new())));
            ...
            "__quantum__rt__callable_invoke" => {
                ...
                graph.Expression(
                    Expression::ArgInjection(
                        method.clone(),
                        if args == Value::Empty { None } else { Some(args) }),
                    None);
            ...
            "__quantum__rt__string_concatenate" => {
                let ref_id = get_ref_id_from_instruction(inst.borrow());
                let left_string = parse_as_value(inst, 0)
                  .expect("Can't resolve string value.");
                let right_string = parse_as_value(inst, 1)
                  .expect("Can't resolve string value.");
            ...
            "__quantum__rt__string_create" => {
                let ref_id = get_ref_id_from_instruction(inst.borrow());
                let created_string = parse_as_value(inst, 0)
                  .expect("Can't resolve string creation target.");
            ...
            _ => { warn!("Attempted to process unknown intrinsic {}.", name)}
        }

        None
    }

    fn eval_call(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let method_name = self.get_method_name(inst.borrow()).expect("Can't resolve method name of call operation.");
        ...
        if called_func.is_none() || called_func.unwrap().get_basic_blocks().len() == 0 {
            ...
            while index < loops {
                ...
                let value = self.as_value_ptr(operand_to_value!(inst, index.clone()), graph, context).expect("Unable to resolve value.");
            ...
            let mut subgraph =
              Ptr::from(CallableAnalysisGraph::new(&builder.graph));
    ...
    fn eval_int_to_ptr(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let val = self.as_value(inst.as_any_value_enum().borrow_mut(), graph, context).expect("Int to pointer unresolvable.");
        let ref_id = get_ref_id_from_instruction(inst.borrow());
        graph.Assign(ref_id, val);
    }

    /// We implicitly convert types on use, so as long as they aren't wildly different no need for static casts.
    fn eval_numeric_cast(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let val = self.as_value(operand_to_value!(inst, 0), graph, context).expect("Int to pointer unresolvable.");
        let ref_id = get_ref_id_from_instruction(inst.borrow());
        graph.Assign(ref_id, val);
    }

    fn eval_bitcast(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let val = self.as_value(operand_to_value!(inst, 0), graph, context).expect("Bitcast value unresolvable.");
        let ref_id = get_ref_id_from_instruction(inst.borrow());
        graph.Assign(ref_id, val);
    }

    fn eval_trunc(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let val = self.as_value(operand_to_value!(inst, 0), graph, context).expect("Truncate value unresolvable.");
        let ref_id = get_ref_id_from_instruction(inst.borrow());
        graph.Assign(ref_id, val);
    }

    /// Load is meaningless for us, as is alignment and memory metadata. Just treat it as an assign.
    fn eval_load(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let val = self.as_value(operand_to_value!(inst, 0), graph, context).expect("Load value unresolvable.");
        let ref_id = get_ref_id_from_instruction(inst.borrow());
        graph.Assign(ref_id, val);
    }

    fn eval_store(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let value = self.as_value(operand_to_value!(inst, 0), graph, context).expect("Store value unresolvable.");

        // The argument resolves to another variable which we want to just directly assign too.
        let target_variable_str = get_ref_id_from_instruction(inst.get_operand(1).unwrap().left().unwrap().as_instruction_value().expect("Has to be storing in another variable.").borrow());
        graph.Assign(target_variable_str, value);
    }

    fn eval_branch(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                   context: &Ptr<EvaluationContext>) {
        let op_count = inst.get_num_operands();
        let branch_basic_block = inst.get_parent().unwrap().get_name().to_str().unwrap().to_string();
                ...
                if let Some(phi) = potential_phi {
                    match phi.get_opcode() {
                        InstructionOpcode::Phi => {
                            ...
                            let capture_groups: Vec<String> = bb_finder.captures_iter(
                                inst_string.as_str()).map(|val| val.get(1).unwrap().as_str().trim().to_string()).collect();
                            ...
                            let mut i = 0;
                            while i < operands {
                                let basic_block = capture_groups.get(i as usize).expect("Can't find the name of the basic block.").clone();
                                if basic_block == branch_basic_block {
                                    let val = self.as_value(operand_to_value!(phi, i), graph, context).expect("Can't resolve phi node references.");
        ...
        if op_count == 1 {
            ...
            let target = with_mutable!(context.anchors.get_mut(target.as_str()).expect("Node should exist."));
            ...
            let condition = self.as_value(
                operand_to_value!(inst, 0),
                graph,
                context).expect("Conditional unable to be evaluated for branch.");
            ...
            with_mutable!(graph.add_edge(last_node.borrow_mut(), true_branch.borrow_mut(), true_assignments,
                Some(Condition::new(condition, Equalities::Equals, Value::Bool(true)))));
            with_mutable!(graph.add_edge(last_node.borrow_mut(), false_branch.borrow_mut(), false_assignments, None));
        }
    }

    fn eval_icmp(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                 context: &Ptr<EvaluationContext>) {
        ...
        let left = self.as_value(
            operand_to_value!(inst, 0), graph, context)
          .expect("Can't resolve left side of icmp.");

        let right = self.as_value(
            operand_to_value!(inst, 1), graph, context)
    ...
    fn add_arithmatic_op(&self, op: Operator, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                         context: &Ptr<EvaluationContext>){
        let lhs = self.as_value(
            operand_to_value!(inst, 0), graph, context)
    ...
    fn eval_mul(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) {
        self.add_arithmatic_op(Operator::Multiply, inst, graph, context);
    }

    fn eval_div(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) {
        self.add_arithmatic_op(Operator::Divide, inst, graph, context);
    }

    fn eval_sub(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) {
        self.add_arithmatic_op(Operator::Subtract, inst, graph, context);
    }

    fn eval_add(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) {
        self.add_arithmatic_op(Operator::Add, inst, graph, context);
    }

    fn eval_insertvalue(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) {
        ...
        let target_composite = self.as_value(operand_to_value!(inst, 0), graph, context)
          .expect("Can't resolve composite to insert into.");

        let insert_value = self.as_value(operand_to_value!(inst, 1), graph, context)
        ...
        for indexer in inst_str.split(',').rev() {
            ...
            if Regex::new("^[0-9]+$").unwrap().is_match(indexer) {
                index_values.push(indexer.parse::<i64>().expect(format!("Unable to parse {} as an int", indexer).as_str()));
            }
        }
        ...
        for index in index_values {
            graph.Assign(throwaway_var.clone(), Value::Ref(target_ref, Some(Ptr::from(Value::Int(index)))));
    ...
    fn eval_extractvalue(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>,
                context: &Ptr<EvaluationContext>) {
        // TODO: Don't double-up stringification from get_ref_x.
        let inst_str = inst.to_string();
        let inst_str = inst_str.trim_matches('"').trim();
        ...
        for index in index_values {
            let next_throwaway = context.next_throwaway();
            graph.Assign(next_throwaway.clone(), Value::Ref(throwaway_var, Some(Ptr::from(Value::Int(index)))));
    ...
    fn eval_getelementptr(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let target_ref = get_ref_id_from_instruction(inst);
        let extracted_ref = self.extract_gep(inst.as_any_value_enum().borrow(), graph, context)
          .expect("Couldn't extract getelementptr instruction.");

        graph.Assign(target_ref.clone(), extracted_ref);
    }

    fn eval_select(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let target_ref = get_ref_id_from_instruction(inst);
        let condition = self.as_value(operand_to_value!(inst, 0), graph, context)
          .expect("Couldn't get target.");

        let true_value = self.as_value(operand_to_value!(inst, 1), graph, context)
          .expect("Couldn't get true select value.");

        let false_value = self.as_value(operand_to_value!(inst, 2), graph, context)
        ...
        let mut added_node = with_mutable!(graph.add_with_edge(
            &attach_node, &mut last_node, Some(vec![(target_ref.clone(), false_value)]), None));

        with_mutable!(graph.add_edge(&mut last_node, &mut added_node, Some(vec![(target_ref, true_value)]),
                       Some(Condition::new(condition, Equalities::Equals, Value::Bool(true)))));
    }

    fn eval_bitwise(&self, op: Operator, inst: &Ptr<InstructionValue>,
                    graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        let target_ref = get_ref_id_from_instruction(inst);
        let lhs = self.as_value(operand_to_value!(inst, 0), graph, context)
          .expect("Couldn't get true select value.");

        let rhs = self.as_value(operand_to_value!(inst, 1), graph, context)
          .expect("Couldn't get false select value.");

        graph.Arithmatic(target_ref, lhs, op, rhs);
    }

    fn eval_or(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        self.eval_bitwise(Operator::Or, inst, graph, context);
    }

    fn eval_and(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        self.eval_bitwise(Operator::And, inst, graph, context);
    }

    fn eval_xor(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        self.eval_bitwise(Operator::Xor, inst, graph, context);
    }

    fn eval_ret(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
        if inst.get_num_operands() == 1 {
            let results=
                  self.as_value(operand_to_value!(inst, 0), graph, context)
    ...
    fn eval_fneg(&self, inst: &Ptr<InstructionValue>, graph: &Ptr<AnalysisGraphBuilder>, context: &Ptr<EvaluationContext>) {
